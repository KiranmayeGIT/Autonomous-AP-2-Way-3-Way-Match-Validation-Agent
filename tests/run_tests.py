...
class TestAPAgent(unittest.TestCase):

    def test_inv_99201_auto_approve(self):
        ...
        self.assertEqual(result.disposition, DispositionType.AUTO_APPROVE)
        self.assertEqual(len(result.holds_applied), 0)
        self.assertEqual(result.line_item_validation[0].price_variance_amount, 30.00)
        self.assertEqual(result.line_item_validation[0].price_variance_percentage, 2.40)

    def test_inv_2026_8804_escalate_holds(self):
        payload = TransactionPayload(
            invoice_header=InvoiceHeader(
        ...
        self.assertEqual(result.disposition, DispositionType.ESCALATE_TO_AP_SPECIALIST)
        self.assertIn("TERMS DISCREPANCY", result.holds_applied)
        self.assertIn("QTY REC", result.holds_applied)
        self.assertIn("PRICE", result.holds_applied)

    def test_unmapped_line_rejection(self):
        payload = TransactionPayload(
            invoice_header=InvoiceHeader(
        ...
        self.assertEqual(result.disposition, DispositionType.SYSTEM_REJECT)
        self.assertIn("UNMAPPED LINE", result.holds_applied)
